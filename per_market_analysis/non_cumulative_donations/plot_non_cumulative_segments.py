#!/usr/bin/env python3
"""
Generate segment-specific plots for non-cumulative donation ratios: all segments together 
and per-segment (Small, Medium, Large) with Polymarket prices on the same 0-1 axis.
"""
...
def _plot_one(
    ...
    plots_dir = slug_dir / "non_cumulative_donations" / "plots"
    ...
    ax.set_title(f"{slug} — Donation period ratio ({segment_name}) + Polymarket ({frequency})", fontsize=14, fontweight="bold", pad=20)
    ...
    out_path = plots_dir / f"non_cumulative_ratio_{frequency}_{out_suffix}.png"
    plt.savefig(out_path, dpi=300, bbox_inches="tight")
    plt.close()
    print(f"  ✓ Saved: {out_path.name}")


def plot_non_cumulative_segments_for_slug(slug_dir: Path, slug: str) -> None:
    """
    Generate non-cumulative segment plots for one slug: all segments on one chart, plus one chart per segment
    ...
    for frequency in ["daily", "weekly", "monthly"]:
        ...
        agg_file = output_dir / f"{frequency}_non_cumulative_aggregations.csv"
        if not agg_file.exists():
            continue
        full_agg = pd.read_csv(agg_file)
        segments_in_file = full_agg["Segment"].unique().tolist() if "Segment" in full_agg.columns else []

        # Plot individual segment charts (All, Small, Medium, Large)
        ...
        ax.set_xlabel(f"{frequency.capitalize()} Period", fontsize=12, fontweight="bold")
        ax.set_ylabel("Ratio / Price (0-1)", fontsize=12, fontweight="bold")
        ax.set_title(f"{slug} — Donation period ratio (all segments) + Polymarket ({frequency})", fontsize=14, fontweight="bold", pad=20)
        ...
        plt.tight_layout()
        plots_dir.mkdir(parents=True, exist_ok=True)
        out_path = plots_dir / f"non_cumulative_ratio_{frequency}_with_polymarket.png"
        plt.savefig(out_path, dpi=300, bbox_inches="tight")
        plt.close()
        print(f"  ✓ Saved: {out_path.name}")
